...
export class AvatarGenerationService {
  ...
  async generateAvatarDetails(prompt) {
    let retry = 3;

    while (retry > 0) {
      try {
        // Use AI service to generate avatar details
        const response = await this.aiService.chat([
          {
            role: "system",
            content: "Generate a creative avatar with personality, description, and emoji."
          },
          {
            role: "user",
            content: prompt
          ...
          fallbackOnError: retry === 1, // Use fallback on last attempt
          fallbackResponse: JSON.stringify({
            name: "Fallback Avatar",
            description: "A mysterious entity that emerged from the digital void.",
            personality: "Enigmatic and resourceful, adapting to challenges with quiet determination.",
            emoji: "✨"
          })
        });

        // Process response
        return this.extractAvatarDetails(response);
      } catch (error) {
        retry--;
        this.logger.error(`Avatar generation error: ${error.message}`);

        if (retry > 0) {
          this.logger.warn(`Retrying avatar generation, ${retry} attempts remaining`);
          // Try a different model for next attempt
          await this.aiService.selectRandomModel(true);
        } else {
          this.logger.warn("Using fallback response due to API error");
