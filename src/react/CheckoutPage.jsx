...
function CheckoutPage() {
  // Memoize URL parameters so they aren’t recalculated on each render
  const params = useMemo(() => new URLSearchParams(window.location.search), []);
  const templateId = params.get("templateId");
  const collectionId = params.get("collectionId");
  const clientId = window.CROSSMINT_CLIENT_API_KEY;

  // If any required parameter is missing, display an error state
  if (!clientId || !templateId || !collectionId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-purple-900">
        <p className="text-red-400 text-xl">Missing required parameters</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-purple-900 py-6 flex flex-col justify-center sm:py-12">
      <div className="relative sm:max-w-xl sm:mx-auto">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-cyan-400 to-blue-500 shadow-lg transform -skew-y-6 sm:skew-y-0 sm:-rotate-6 sm:rounded-3xl opacity-75 transition duration-700"></div>
