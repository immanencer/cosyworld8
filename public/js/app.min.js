...
function AvatarModal(_ref2) {
  ...
  useEffect(() => {
    if (avatar !== null && avatar !== void 0 && avatar._id && isOpen) {
      Promise.all([fetch("/api/avatars/".concat(avatar._id, "/narratives")).then(r => r.json()), fetch("/api/avatars/".concat(avatar._id, "/memories")).then(r => r.json()), fetch("/api/avatars/".concat(avatar._id, "/location")).then(r => r.json()), fetch("/api/avatars/".concat(avatar._id, "/actions")).then(r => r.json())]).then(_ref3 => {
        let [narrativeData, memoryData, locationData, actionsData] = _ref3;
        setActivityData({
